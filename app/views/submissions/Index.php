                ...
                <div class="col-md-6 col-lg-4">
                    <div class="card h-100 shadow-sm border-0 rounded-4 overflow-hidden hover-shadow transition-all">
                        <div class="position-relative" style="height: 250px;">
                            <?php 
                            // Correction du chemin d'image : on vérifie les deux clés possibles et on nettoie le slash
                            $photoPath = $submission['image'] ?? $submission['image_path'] ?? '';
                            if(!empty($photoPath)) : ?>
                                <img src="<?= ltrim(htmlspecialchars($photoPath), '/'); ?>" class="w-100 h-100 object-cover" style="object-fit: cover;">
                            <?php else: ?>
